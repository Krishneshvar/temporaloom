...
export default function BenchmarkChart({ data, loading }) {
  if (loading) {
    return (
      <div className="h-full bg-[var(--surface)] rounded-2xl border border-[var(--border)] flex flex-col items-center justify-center p-8 min-h-[400px]">
         ...
         <span className="mt-8 text-[var(--foreground)]/70 font-bold uppercase tracking-widest text-sm">Evaluating Performance Envelope...</span>
         <p className="text-xs text-[var(--text-muted)] mt-2 max-w-sm text-center">Executing 4-stage hardware iterations. This may take longer on large scale topologies.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-[var(--surface)] rounded-2xl border border-[var(--border)] shadow-2xl p-6 min-h-[400px]">
       <div className="flex items-center gap-3 mb-8">
           <div className="p-2 bg-purple-500/10 rounded-lg border border-purple-500/20 text-purple-400">
               <Zap size={18} />
           </div>
           <div>
               <h3 className="text-[var(--foreground)] font-bold text-lg leading-tight">Hardware Benchmark Profiles</h3>
               <p className="text-[var(--text-muted)] text-xs font-semibold">Comparing execution vectors across CPU and CUDA paradigms</p>
           </div>
       </div>
            ...
            return (
              <div key={result.id} className="flex flex-col gap-2">
                 <div className="flex justify-between items-end">
                    <span className="text-sm font-bold flex items-center gap-2 text-[var(--foreground)]/90">
                    ...
                    <div className="flex items-center gap-3">
                       ...
                       <span className={`text-xs font-mono font-bold ${hasError ? 'text-red-400' : 'text-[var(--text-dim)]'}`}>
                          {hasError ? 'FAILED' : `${execTime.toFixed(4)}s`}
                       </span>
                    </div>
                 </div>
                 
                 <div className="w-full bg-[var(--border)] h-4 rounded-full overflow-hidden relative">
                    {!hasError && (
                       <motion.div 
                          initial={{ width: 0 }}
       ...
       {data.every(d => !d.error) && data.length > 2 && (
         <div className="mt-8 p-4 bg-[var(--surface-hover)] rounded-xl border border-[var(--border)] flex justify-between items-center">
            <div className="flex flex-col">
               <span className="text-[10px] text-[var(--text-muted)] uppercase font-bold tracking-widest">Top Performer</span>
               <span className="text-[var(--foreground)] font-bold">{data.reduce((prev, curr) => (prev.data.execution_time < curr.data.execution_time) ? prev : curr).name}</span>
            </div>
            <div className="w-[1px] h-8 bg-[var(--border)]" />
            <div className="flex flex-col text-right">
               <span className="text-[10px] text-[var(--text-muted)] uppercase font-bold tracking-widest">Max Delta</span>
